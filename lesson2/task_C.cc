...
class Segment_tree {
 ...
 public:
  ...
  pair<int, int> segment_ind_max(int left, int right) {
    pair<int, int> result = segment_ind_max(
        size / 2 + left - 1, size / 2 + right - 1, 0, size / 2, size - 1);
    result.second += 1;
...
int main() {
  ...
  for (int i = 0; i < k; i++) {
    auto res = data.segment_ind_max(segments[i].first, segments[i].second);
